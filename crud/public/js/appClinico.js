...
window.addEventListener('DOMContentLoaded', async (e) => {
  await pacienteRef.orderByChild('especialidad').equalTo('Clínico').on('value', (pacientes) => {
  ...
  if (textoBusqueda === "") {
    ...
    pacienteRef.on("value", function (snapshot) {
      ...
      snapshot.forEach(function (childSnapshot) {
        ...
        if (pacienteData.especialidad === "Clínico") { // verificar si la propiedad especialidad es igual a Kinesiologia
        table.innerHTML += `
          <tr>
            <td>${pacienteData.nombre}</td>
            <td>${pacienteData.apellido}</td>
    ...
    pacienteRef.orderByChild("apellido").on("value", function(snapshot) {
      ...
      snapshot.forEach(function(childSnapshot) {
        var pacienteKey = childSnapshot.key;
        var pacienteData = childSnapshot.val();
        if (pacienteData.especialidad === "Clínico") {
